        ...
        {isFullPlayerOpen && currentPlayingItem && (
            ...
            >
                <div className="flex items-center justify-between px-6 pt-6 pb-2">
                    <button onClick={() => setIsFullPlayerOpen(false)} className="p-2 rounded-full bg-zinc-800/50 hover:bg-zinc-700/50 transition-colors">
                        <ChevronDown size={24} />
                    </button>
                    <span className="text-xs text-zinc-500 font-medium uppercase tracking-widest">Now Playing</span>
                    <div className="w-10" />
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6">
                    ...
                    >
                        {currentPlayingItem.customIcon || currentPlayingItem.coverUrl ? (
                            <img src={currentPlayingItem.customIcon || currentPlayingItem.coverUrl} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <div className="w-full max-w-md text-center">
                        <h2 className="text-xl font-bold text-white truncate">{(currentPlayingItem as any).decryptedName || currentPlayingItem.name}</h2>
                        <p className="text-sm text-zinc-400 truncate mt-1">{(currentPlayingItem as any).decryptedArtist || currentPlayingItem.artist || 'Unknown Artist'}</p>
                    </div>
                    ...
                    <div className="w-full max-w-md">
                        ...
                        <div className="flex justify-between mt-1">
                            <span className="text-xs text-zinc-500 font-mono">{Math.floor(currentTime / 60)}:{String(Math.floor(currentTime % 60)).padStart(2, '0')}</span>
                            <span className="text-xs text-zinc-500 font-mono">{Math.floor(duration / 60)}:{String(Math.floor(duration % 60)).padStart(2, '0')}</span>
                        </div>
                    </div>
