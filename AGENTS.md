...
- **Crypto is off-limits**: Never modify `utils/crypto.ts`, `utils/cryptoPrimitives.ts`, `utils/streamCrypto.ts`, `utils/backupCrypto.ts`, `utils/metadataCrypto.ts`, or any encryption logic. Only the project architect and approved external security auditors handle cryptographic code.
